...
export class PaymentController {
  ...
  @Post(':paymentId/confirm-transfer')
  @ApiOperation({ summary: 'Manual transfer confirmation fallback for VietQR' })
  @ApiResponse({ status: 200, description: 'Transfer payment confirmed.' })
  @ApiBody({ type: ConfirmTransferDto })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @HttpCode(HttpStatus.OK)
  confirmTransfer(@Param('paymentId') paymentId: string, @Body() dto: ConfirmTransferDto) {
    return this.paymentService.confirmTransferPayment(paymentId, dto.transactionId);
  }
}
